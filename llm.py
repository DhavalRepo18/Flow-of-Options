...
import config as cfg  # assuming you have the same cfg object
...
def query_programmer(
    input: str, do_cache: bool = cfg.SEED is not None
) -> tuple[dict, float]:
...
def query_metric_summarizer(
    task: str, result: str, get_metric: bool = False
) -> tuple[float, str]:
    prompt, prompt_dir = generate_metric_summarize_prompt(task, result)
    metric = call_model_litellm(prompt)[0]
    metric_dir = call_model_litellm(prompt_dir)[0] if get_metric else None
...
def query_debugger(
    ...
    while return_code != 0:
        if error_iter == max_error_iterations:
            print(
                f"Failed to debug walk {walk_num} in {max_error_iterations} iters... increase max_error_iterations if needed."
            )
        ...
        log_response(
            log_folder,
            f"Code ran into an issue: {corrected_response['Explanation']}\n",
            costs,
        )
        save_generated_code(
            log_folder, "testing.py", corrected_response["Code"]
        )  # Save corrected code
...
def generate_code(
    task: str, plan: str, implementation: list[str], log_folder: str, code_fname: str
) -> str:
...
def call_model_openai(
    prompt: str, model: str = cfg.MODEL, temperature: float = 0.7
) -> tuple[dict, float]:
...
def call_model_litellm(
    ...
    # LiteLLM usage info: can estimate token counts and costs
    ...
    if usage:
        ...
        # If you have a token price mapping:
        price_per_1k_prompt = cfg.PRICES.get(model, {}).get("prompt", 0)
        price_per_1k_completion = cfg.PRICES.get(model, {}).get("completion", 0)
        costs = (prompt_tokens / 1000) * price_per_1k_prompt + (
